...
const FloatingChatbot = () => {
  ...
  const handleSend = async (customInput = null) => {
    ...
    try {
      ...
      const response = await sendChatMessage(text, analysisResults || []);

      setMessages(prev => [...prev, { role: 'bot', text: response }]);
    } catch (error) {
      setMessages(prev => [...prev, { role: 'bot', text: "⚠️ Could not reach the AI assistant. Make sure the backend is running (`npm run dev:backend`)." }]);
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="fixed bottom-8 right-8 z-[100]">
          ...
          >
            ...
            <div ref={scrollRef} className="flex-1 p-8 overflow-y-auto space-y-8 bg-gray-50/10 scroll-smooth">
              {messages.map((m, i) => (
                ...
                >
                  <div className={`max-w-[85%] p-6 rounded-[2rem] shadow-sm ${m.role === 'user'
                    ? 'bg-primary text-white rounded-tr-none'
                    : 'bg-white border border-gray-100 text-gray-700 rounded-tl-none font-medium'
                    }`}>
                    <MarkdownMessage text={m.text} isUser={m.role === 'user'} />
                  </div>
                </motion.div>
              ))}
      ...
      <div className="relative">
        ...
        <motion.button
          ...
          className={`w-18 h-18 rounded-[2rem] flex items-center justify-center shadow-[0_15px_40px_-10px_rgba(22,163,74,0.4)] transition-all duration-500 overflow-hidden ${isOpen ? 'bg-gray-900 text-white' : 'bg-primary text-white'
            }`}
        >
